...
```powershell
.\start\dsh-plugin-reinstall.bat 2.0.2
```
...
dsh plugin --profile web add dsh-xiaomi-tts@2.0.2
.\start\dsh-web-start.bat
```

When `mimo-v2.5-tts-voicedesign` is selected, the plugin omits `audio.voice`, sends the custom voice description as the upstream `user` message, and sends the reply text as the `assistant` message. Describe the voice itself in one or two sentences, including age/gender, texture, pace, and emotional baseline; avoid scenes or actions.
