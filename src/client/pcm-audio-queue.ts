...
export class PcmAudioQueue {
  ...
  enqueue(base64: string): Promise<void> {
    ...
    debugConsole?.info(this.logPrefix, '[入队] 收到 PCM Base64', { revision, base64Chars: base64.length })
  ...
  stop(): void {
    debugConsole?.info(this.logPrefix, '[停止] 清空播放队列', { sources: this.sources.size, queuedBytes: this.queuedBytes })
    this.revision += 1
    this.scheduledAt = 0
  ...
  private async schedule(base64: string, revision: number): Promise<void> {
    if (revision !== this.revision) {
      debugConsole?.warn(this.logPrefix, '[调度] 已跳过过期 PCM 块', { chunkRevision: revision, currentRevision: this.revision })
      return
    }
    const bytes = Uint8Array.from(atob(base64), (character) => character.charCodeAt(0))
    if (bytes.byteLength < 2) {
      debugConsole?.warn(this.logPrefix, '[解码] 已跳过空 PCM 块', { bytes: bytes.byteLength })
      return
    }
    const context = this.getContext()
    debugConsole?.info(this.logPrefix, '[上下文] 当前 AudioContext 状态', { state: context.state, userPaused: this.userPaused })
    if (context.state !== 'running' && !this.userPaused) {
      debugConsole?.info(this.logPrefix, '[上下文] 尝试 resume()')
      await context.resume()
      debugConsole?.info(this.logPrefix, '[上下文] resume() 已返回', { state: context.state })
    }
    ...
    source.addEventListener('ended', () => {
      ...
      debugConsole?.info(this.logPrefix, '[播放] PCM 音频节点已结束', { remainingSources: this.sources.size, queuedBytes: this.queuedBytes })
      if (this.sources.size === 0) this.setBusy(false)
    }, { once: true })
    try {
      source.start(startAt)
      debugConsole?.info(this.logPrefix, '[播放] AudioBufferSourceNode.start() 已调用', { startAt })
    } catch (error) {
      this.sources.delete(source)
      this.queuedBytes -= this.sourceBytes.get(source) ?? 0
      this.sourceBytes.delete(source)
