...
function finalLiveMessage(snapshot, turn, step) {
	for (let index = snapshot.nodes.length - 1; index >= 0; index -= 1) {
		const node = snapshot.nodes[index];
		if (node?.kind === "assistant" && node.turn === turn && node.step === step) return {
			turn: node.turn,
			step: node.step,
...
var LiveSpeechController = class {
	audio = new PcmAudioQueue();
	queue = new AbortableSentenceQueue((sentence, signal) => this.stream(sentence, signal));
	active = null;
	observed = "";
	consumed = 0;
	pendingText = "";
...
function AutoPlayRunObserver({ sessionId, session, playback }) {
...
function LiveSpeechObserver({ sessionId, session, live, settings }) {
	...
	(0, react.useEffect)(() => () => live.cancelSession(sessionId), [live, sessionId]);
	return null;
}
function ReadAloudAction({ sessionId, messageId, useSession, playback, live, settings, t }) {
	...
	(0, react.useEffect)(() => {
		...
		const cancel = window.setTimeout(() => {
			if (!live.hasHandled(sessionId, message.identity) && playback.claimAutomaticPlayback(sessionId, messageId)) playback.toggle(messageId, text, true);
		}, 0);
