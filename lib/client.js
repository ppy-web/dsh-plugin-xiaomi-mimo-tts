...
var LiveSpeechController = class {
	...
	beginSegment(next, preserveMessageId) {
		this.active = next;
		this.observed = "";
		this.consumed = 0;
		this.pendingText = "";
...
var PlaybackController = class {
	...
	updateLivePlayback(sessionId, messageId, status) {
		...
		if (this.view.source !== "complete") this.publish({
			sessionId,
			messageId,
			source: "live",
			status,
	...
	publish(view) {
		if (this.view.sessionId === view.sessionId && this.view.messageId === view.messageId && this.view.source === view.source && this.view.status === view.status && this.view.error === view.error) return;
		this.view = view;
...
function ReadAloudAction({ sessionId, messageId, useSession, playback, live, settings, t }) {
	...
	const label = status === "loading" ? liveActive ? t("action.cancel") : t("action.loading") : status === "playing" ? liveActive ? t("action.stop") : t("action.pause") : status === "paused" ? t("action.resume") : status === "error" ? t("action.retry") : t("action.play");
