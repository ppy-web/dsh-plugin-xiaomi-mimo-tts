import { a as TTS_ROUTE, d as prepareTtsText, i as TTS_MODELS, n as DEFAULT_TTS_SETTINGS, o as TTS_SETTINGS_NAMESPACE, r as TTS_FORMATS, s as TTS_STREAM_ROUTE } from "./shared-DV-2WugB.js";
...
function requestMessages(options, text) {
	const context = options.model === "mimo-v2.5-tts-voicedesign" ? [options.voiceDesignPrompt.trim(), options.instruction.trim()].filter((item) => item.length > 0).join("\n") : options.instruction.trim();
	const messages = [];
